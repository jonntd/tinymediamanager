...
public class BatchChatGPTEpisodeRecognitionService {
    ...
    public Map<String, EpisodeMatchingResult> batchRecognizeEpisodes(List<TvShowEpisode> episodes) {
        ...
        for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
            ...
            LOGGER.info("Processing episode batch {}/{} ({} episodes)", batchIndex + 1, totalBatches, currentBatch.size());
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (contentStart != -1) {
                        ...
                        if (contentEnd != -1) {
                            String result = responseBody.substring(contentStart, contentEnd).replace("\\\"", "\"").replace("\\n", "\n").trim();
