...
public class TvShowAIPromptTemplates {
    ...
    public static String getBatchTvShowRecognitionPrompt() {
        ...
        return "你是电视剧识别专家。遇到不规则或难以识别的文件名时，**必须利用联网搜索工具**分析文件特征（如别名、演员、特定关键词）以确定其真实身份。\n\n" + "## ⚠️ 严格禁止（违反将导致识别失败）\n"
                ...
                + "- **严禁使用自造的直译名**，必须是官方存在的标题。\n" + "- **年份**：必须是电视剧首播年份（从 TMDB/TVDB 获取），不是文件名中的年份。\n\n" + "## 输出格式要求\n" + "每行格式：`<<序号>> 标题 年份`\n"
                + "- 序号必须与输入完全对应\n" + "- 每行只输出一个结果，包含标题和年份，用空格分隔\n" + "- 无法识别时：直接返回从文件名中提取的原始标题和年份（去除技术标签后的核心名称）\n"
                + "- **必须联网验证**：确保标题和年份与 TMDB/TVDB 记录完全匹配。\n\n" + "## 示例\n" + "输入：\n" + "<<1>> /TV/Breaking.Bad.S01[1080p]/\n"
                + "<<2>> /TV/合集[1]/权力的游戏.S08/\n" + "<<3>> /Series/Sense8.S01/\n" + "<<4>> /TV/成家（2025）完结/\n" + "输出：\n" + "<<1>> 绝命毒师 2008\n"
                + "<<2>> 权力的游戏 2011\n" + "<<3>> 超感猎杀 2015\n" + "<<4>> 成家 2025";
    }
